...
void benchmarkAccuracyPoints(Model model, int output_size){
    ...
    log_debug("red_count = %d\n", red_count);
    log_debug("blue_count = %d\n", blue_count);
...
void benchmarkAccuracyAndTime(Model model, float **inputs, int nb_samples, int output_size) {
...
int benchmarkSpiral(Model model) {
    ...
    benchmarkAccuracyAndTime(model, inputs, SAMPLE_SIZE, 2);
