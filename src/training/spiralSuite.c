...
int trainOnSpiral(Model model, float training_step, int batch_size) {
    ...
    if(access(COLOR_VALUE_FILE, F_OK) != 0) {
        ...
        generateColorFile(SPIRAL_VALUE_FILE, COLOR_VALUE_FILE);
        printf("Generation complete\n");
    } else {
        printf("Cached color values found.\n");
    }

    printf("Precomputing training values...\n");
    srand(time(NULL));
    ...
    for(int i = 0 ; i < batch_size ; i++) {
        ...
        int r;
        int b;
        determineColor(COLOR_VALUE_FILE, x, y, &r, &b);
    ...
    printf("Precomputing complete.\n");

    printf("Training...\n");
    int ret = train(model, training_step, batch_size, inputs, 2, expecteds, 2);
    if(ret != 0) {
        printf("Error during training. Stopping.\n");
        return -1;
    }
